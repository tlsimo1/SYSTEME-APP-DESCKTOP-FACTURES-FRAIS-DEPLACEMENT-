            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    SqlDataAdapter da = new SqlDataAdapter("select f.Num_Fac as 'Numero Facture',f.Fournisseur,f.Date_Fac,b.Date_Saisie," +
                        ...
                        " left join Cmp c on c.Fac_Num =f.Num_Fac where ad.Statut_AssDaf =0;", connection);
        ...
        public void Edit_Status(string facNum)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    ...
                    cmd2.CommandText = "UPDATE Ass_DAF SET Statut_AssDaf = 1 , Date_Saisie= '" + DateTime.Now.ToString("dd/MM/yyyy") + "' WHERE Fac_Num = '" + facNum + "'";
