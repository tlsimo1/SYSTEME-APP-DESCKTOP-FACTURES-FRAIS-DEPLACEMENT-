...
{
    public class AssDAF_DAL
    {
        public DataTable GetAllFactureAssDAF()
        {
                ...
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    SqlDataAdapter da = new SqlDataAdapter(@"
                        ...
                        select f.Num_Fac as [Numero Facture],
                         ...
                         where ad.Statut_AssDaf = 0;", connection);
        ...
        public void Edit_Status(string facNum)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    ...
                    cmd2.CommandText = "UPDATE Ass_DAF SET Statut_AssDaf = 1 , Date_Saisie= '" + DateTime.Now.ToString("dd/MM/yyyy") + "' WHERE fac_id = '" + facNum + "'";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    SqlDataAdapter da = new SqlDataAdapter(@"
                         ...
                         left
                         join Chef_Cmp cf on cf.fac_id = f.ID_FAC
                         where ad.Statut_AssDaf = 0   and f.Num_Fac  like '%" + facNum + "%'", connection);
                    da.Fill(dt);
                }
            }
            catch (Exception e)
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
                {
                    SqlDataAdapter da = new SqlDataAdapter(@"
                         ...
                         left
                         join Chef_Cmp cf on cf.fac_id = f.ID_FAC
                         where ad.Statut_AssDaf = 0 and f.Date_Fac = '" + date + "' ", connection);
